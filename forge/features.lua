...
function features.playerIsLookingAt(target, sensitivity, zOffset)
    ...
    local function read_vector3d(Address)
        return read_float(Address), read_float(Address + 0x4),
               read_float(Address + 0x8)
    end
    ...
    if m_target_object and m_object then -- Both objects must exist.
        ...
        local target_x, target_y, target_z =
            read_vector3d(m_target_object + 0x5C) -- target Location2
        local distance = math.sqrt((target_x - player_x) ^ 2 +
                                       (target_y - player_y) ^ 2 +
                                       (target_z - player_z) ^ 2) -- 3D distance
        ...
        if average < auto_aim then return true end
    end
    return false
end
